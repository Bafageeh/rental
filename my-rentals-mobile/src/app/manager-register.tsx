...
export default function ManagerRegisterScreen() {
  ...
  async function requestOtp() {
    ...
    try {
      ...
      setStep('otp');
      Alert.alert('تم الإرسال', result?.data?.phone_masked ? `تم إرسال رمز التحقق إلى ${result.data.phone_masked}` : 'تم إرسال رمز التحقق.');
    } catch (e) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View style={styles.card}>
            ...
            <Text style={styles.subtitle}>هذا الخيار مخصص لمدير العقارات الجديد. سيتم التحقق من رقم الجوال عبر رمز تحقق ثم تعيين رقم سري للدخول مستقبلًا.</Text>
            ...
            {step === 'phone' ? (
              <>
                ...
                <TouchableOpacity style={styles.btn} onPress={requestOtp} disabled={loading} activeOpacity={0.85}>
                  {loading ? <ActivityIndicator color={colors.textInverse} /> : <Text style={styles.btnText}>إرسال رمز التحقق</Text>}
                </TouchableOpacity>
              </>
            ) : step === 'otp' ? (
              <>

...
function Contact({ icon, label, value }: { icon: keyof typeof Ionicons.glyphMap; label: string; value: unknown }) {
  return <View style={styles.contact}><Ionicons name={icon} size={15} color="#0F766E" /><Text style={styles.contactLabel}>{label}</Text><Text numberOfLines={1} style={styles.contactValue}>{short(value, label === "البريد" ? 14 : 18)}</Text></View>;
}
...
function Metric({ icon, label, value, green, danger, onPress }: { icon: keyof typeof MaterialCommunityIcons.glyphMap; label: string; value: string; green?: boolean; danger?: boolean; onPress?: () => void }) {
  ...
  return <Box onPress={onPress} activeOpacity={0.86} style={[styles.metric, green && styles.metricGreen, danger && styles.metricDanger]}><View style={[styles.metricIcon, danger && styles.metricIconDanger]}><MaterialCommunityIcons name={icon} size={17} color={danger ? "#DC2626" : "#0F766E"} /></View><Text numberOfLines={1} style={[styles.metricValue, danger && styles.metricValueDanger]}>{value}</Text><Text numberOfLines={1} style={[styles.metricLabel, danger && styles.metricLabelDanger]}>{label}</Text></Box>;
}

function PropertyMini({ icon, label, value }: { icon: keyof typeof MaterialCommunityIcons.glyphMap; label: string; value: string }) {
  return <View style={styles.propertyMini}><MaterialCommunityIcons name={icon} size={15} color="#0F766E" /><Text style={styles.propertyMiniText}>{label}: {value}</Text></View>;
}
...
export default function OwnerAssetsDashboardCompactScreen({ id }: { id: string | number }) {
  ...
  useEffect(() => { const s = BackHandler.addEventListener("hardwareBackPress", () => { goOwners(); return true; }); return () => s.remove(); }, []);
  ...
  function addManual() { router.push(`/property-form?owner_id=${ownerId}&owner_name=${ownerNameUrl}&management_type=owned&owner_private=1&return_to=${returnTo}` as never); }
  function addPdf() { router.push(`/upload-property-deed?owner_id=${ownerId}&owner_name=${ownerNameUrl}&management_type=owned&owner_private=1&return_to=${returnTo}` as never); }
  function addProperty() { setMenu(false); Alert.alert("إضافة عقار", "اختر طريقة الإضافة", [{ text: "رفع PDF", onPress: addPdf }, { text: "إدخال يدوي", onPress: addManual }, { text: "إلغاء", style: "cancel" }]); }
  function overdueUnits() { setMenu(false); router.push(`/owner-overdue-units?owner_id=${ownerId}&owner_name=${ownerNameUrl}` as never); }

  return <SafeAreaView style={styles.safe} edges={["top"]}><ScrollView style={styles.scroll} contentContainerStyle={styles.container} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => load(true)} tintColor="#0F766E" />}>
    <View style={styles.profile}>
      <View style={styles.actions}><TouchableOpacity style={styles.action} onPress={() => setMenu(v => !v)}><Ionicons name="ellipsis-vertical" size={19} color="#111827" /></TouchableOpacity><TouchableOpacity style={[styles.action, styles.wallet]} onPress={ownerAccount}><Ionicons name="wallet-outline" size={20} color="#0F766E" /></TouchableOpacity></View>
      {menu ? <View style={styles.menu}><TouchableOpacity style={styles.menuItem} onPress={ownerDetails}><Ionicons name="eye-outline" size={18} color="#0F766E" /><Text style={styles.menuText}>تفاصيل</Text></TouchableOpacity><TouchableOpacity style={styles.menuItem} onPress={ownerAccount}><Ionicons name="wallet-outline" size={18} color="#0F766E" /><Text style={styles.menuText}>حساب المالك</Text></TouchableOpacity><TouchableOpacity style={styles.menuItem} onPress={ownerEdit}><Ionicons name="create-outline" size={18} color="#0F766E" /><Text style={styles.menuText}>تعديل</Text></TouchableOpacity><TouchableOpacity style={styles.menuItem} onPress={ownerDelete}><Ionicons name="trash-outline" size={18} color="#DC2626" /><Text style={[styles.menuText, styles.dangerText]}>حذف</Text></TouchableOpacity></View> : null}
      <View style={styles.profileRow}><View style={styles.avatar}><Ionicons name="person-outline" size={24} color="#0F766E" /></View><View style={styles.nameBox}><Text numberOfLines={2} style={styles.name}>{ownerName}</Text><Text style={styles.caption}>حساب مالك</Text></View><View style={styles.badge}><Text style={styles.badgeText}>مالك</Text></View></View>
      <View style={styles.contacts}><Contact icon="call-outline" label="الجوال" value={owner?.phone} /><Contact icon="mail-outline" label="البريد" value={owner?.email} /><Contact icon="id-card-outline" label="الهوية" value={owner?.national_id} /></View>
    </View>

    <View style={styles.metrics}><Metric icon="office-building-outline" label="العقارات" value={count(summary.properties_count ?? properties.length)} green /><Metric icon="home-city-outline" label="الوحدات" value={count(summary.units_count ?? units.length)} /><Metric icon="file-document-check-outline" label="العقود" value={count(summary.active_contracts_count)} /><Metric icon="cash-alert" label="المتأخر" value={overdue > 0 ? money(overdue) : "٠ ريال"} danger={overdue > 0} onPress={overdueUnits} /></View>
    ...
    {!loading && !error && tab === "properties" ? <View><View style={styles.sectionRow}><View><Text style={styles.sectionTitle}>عقارات المالك</Text><Text style={styles.sectionSub}>{count(properties.length)} عقار</Text></View><TouchableOpacity style={styles.addBtn} onPress={addProperty}><Ionicons name="add" size={20} color="#fff" /><Text style={styles.addText}>إضافة</Text></TouchableOpacity></View>{properties.map((p) => <TouchableOpacity key={p.id} style={styles.propertyCard} activeOpacity={0.9} onPress={() => router.push(`/property/${p.id}` as never)}><View style={styles.propertyHead}><View style={styles.openIcon}><Ionicons name="chevron-back" size={18} color="#0F766E" /></View><View style={styles.propertyTitleBox}><View style={styles.propertyTitleRow}><Text style={styles.type}>{typeLabels[p.property_type || ""] || p.property_type || "عقار"}</Text><Text numberOfLines={1} style={styles.propertyTitle}>{p.name || "عقار بدون اسم"}</Text></View><View style={styles.location}><Ionicons name="location-outline" size={13} color="#6B7280" /><Text style={styles.locationText}>{[p.district, p.city].filter(Boolean).join("، ") || "لا يوجد موقع مسجل"}</Text></View></View><View style={styles.buildingIcon}><MaterialCommunityIcons name="office-building-outline" size={20} color="#0F766E" /></View></View><View style={styles.propertyMetrics}><PropertyMini icon="home-outline" label="وحدات" value={count(p.units_count)} /><PropertyMini icon="account-key-outline" label="مؤجرة" value={count(p.rented_units_count)} /><PropertyMini icon="file-document-outline" label="عقود" value={count(p.active_contracts_count)} /></View></TouchableOpacity>)}{!properties.length ? <View style={styles.empty}><Text style={styles.emptyText}>لا توجد عقارات تابعة لهذا المالك.</Text></View> : null}</View> : null}

...
export default function MyAccountScreen() {
  ...
  async function load() {
    try {
      ...
      const scoped = await apiGet("/auth/scoped-dashboard");

      const meUser = me?.data ?? me?.user ?? me;
      const scopedDashboard = scoped?.data ?? scoped;

      setUser(meUser);
      setDashboard(scopedDashboard as Dashboard);
    } catch (e) {
      ...
      >
        ...
        {loading ? (
          <View style={styles.box}>
            <ActivityIndicator />
            <Text style={styles.boxText}>جاري تحميل الحساب...</Text>
          </View>
        ) : null}
          ...
          <>
            <View style={styles.card}>
              ...
              <Text style={styles.detail}>
                الصلاحية: {user.role === "admin" ? "مدير" : "مالك"}
              </Text>
            ...
            <View style={styles.grid}>
              <Card title="الملاك" value={summary.owners_count} />
              ...
              <Card title="المتأخرة" value={summary.overdue_count} />
              <Card title="المدفوع" value={money(summary.total_paid)} />
              <Card title="المستحق" value={money(summary.total_due)} />
              <Card title="المصاريف" value={money(summary.total_expenses)} />
